    ...
    public static void handle(OpenBackSlotPacket packet, Supplier<NetworkEvent.Context> ctx) {
        ctx.get().enqueueWork(() -> {
            ...
            if (CuriosCompat.isLoaded()) {
                backpackStack = CuriosCompat.getBackStack(player);
            }

            if (backpackStack.isEmpty()) {
                var capOpt = player.getCapability(BackSlotCapabilityProvider.BACK_SLOT).resolve();
                if (capOpt.isPresent()) {
                    backpackStack = capOpt.get().getBackStack();
                }
            }
            ...
            NetworkHooks.openScreen(
                    ...
                    new SimpleMenuProvider(
                            (id, inv, p) -> new BackpackMenu(
                                    ModMenus.MENUS.get(tier).get(), id, inv, tier, finalStack),
